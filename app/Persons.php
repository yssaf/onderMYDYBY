...
class Persons extends Model
{
     ...
     *
     */
    protected $fillable = ['forename', 'surname', 'birthday', 'deceased', 'bio', 'image', 'imdb_id'];
    ...
    {
        return $this->belongsToMany('App\Movies', 'casts', 'person_id', 'movie_id')
    ...
    {
        return $this->belongsToMany('App\Movies', 'crews', 'person_id', 'movie_id')
    ...
    {
        return DB::select(
            DB::raw('SELECT `casts`.`movie_id` , `persons`.`person_id`, count(*) AS count, CONCAT(`forename`, " ", `surname`) AS name, `image` FROM `persons`
			INNER JOIN `casts` ON `casts`.`person_id` = `persons`.`person_id`
			WHERE(
				SELECT count(*) FROM tags
				WHERE `tags`.`movie_id` = `casts`.`movie_id`
    ...
    public static function getDirectorCount($limit)
    {
        return Persons::select('persons.person_id', DB::raw('count(*) as count'), DB::raw('CONCAT(forename, " ", surname) as name'), 'image')
            ->join('crews', 'crews.person_id', '=', 'persons.person_id')
            ->where('crews.position','director')
            ->groupBy('crews.person_id')
            ->orderBy('count', 'desc')
