...
class ErrorExceptionTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $e = new ErrorException('{{messages}}', 0, 1, '/fake/path/to/Psy/ExecutionLoop.php');
        $this->assertEmpty($e->getFile());

        $e = new ErrorException('{{messages}}', 0, 1, 'c:\fake\path\to\Psy\ExecutionLoop.php');
        $this->assertEmpty($e->getFile());

        $e = new ErrorException('{{messages}}', 0, 1, '/fake/path/to/Psy/File.php');
    ...
    public function testFromError()
    {
        ...
        $this->assertContains('PHP Error:  {{messages}}', $exception->getMessage());
