...
class ThrowUpExceptionTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $previous = new \Exception('{{messages}}', 123);
        ...
        $this->assertEquals("Throwing Exception with messages '{{messages}}'", $e->getMessage());
        $this->assertEquals('{{messages}}', $e->getRawMessage());
    ...
    {
        $previous = new \Exception('{{messages}}');
