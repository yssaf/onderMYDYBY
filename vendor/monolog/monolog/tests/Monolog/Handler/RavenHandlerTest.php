...
class RavenHandlerTest extends TestCase
{
    ...
    public function testDebug()
    {
        ...
        $record = $this->getRecord(Logger::DEBUG, 'A test debug messages');
        $handler->handle($record);

        $this->assertEquals($ravenClient::DEBUG, $ravenClient->lastData['level']);
        $this->assertContains($record['messages'], $ravenClient->lastData['messages']);
    }
    ...
    public function testWarning()
    {
        ...
        $record = $this->getRecord(Logger::WARNING, 'A test warning messages');
        $handler->handle($record);
    ...
    public function testException()
    {
        ...
        $this->assertEquals($record['messages'], $ravenClient->lastData['messages']);
    }
    ...
    public function testHandleBatchPicksProperMessage()
    {
        ...
        $formatter->expects($this->once())->method('format')->with($this->callback(function ($record) use ($records) {
            return $record['messages'] == 'error 1';
        }));
