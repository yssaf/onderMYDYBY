...
class HipChatHandlerTest extends TestCase
{
    ...
    public function testWriteHeader()
    {
        ...
        $this->assertRegexp('/POST \/v1\/rooms\/messages\?format=json&auth_token=.* HTTP\/1.1\\r\\nHost: api.hipchat.com\\r\\nContent-Type: application\/x-www-form-urlencoded\\r\\nContent-Length: \d{2,4}\\r\\n\\r\\n/', $content);
    ...
    {
        $this->assertRegexp('/notify=0&messages=test1&message_format=text&color=red&room_id=room1&from=Monolog$/', $content);
    }
    ...
    public function testWriteContentV1WithoutName()
    {
        ...
        $this->assertRegexp('/notify=0&messages=test1&message_format=text&color=red&room_id=room1&from=$/', $content);
    ...
    {
        $this->assertRegexp('/notify=1&messages=test1&message_format=text&color=red&room_id=room1&from=Monolog$/', $content);
    }
    ...
    {
        $this->assertRegexp('/notify=false&messages=test1&message_format=text&color=red&from=Monolog$/', $content);
    }
    ...
    {
        $this->assertRegexp('/notify=true&messages=test1&message_format=text&color=red&from=Monolog$/', $content);
    }
    ...
    public function testWriteContentV2WithoutName()
    {
        ...
        $this->assertRegexp('/notify=false&messages=test1&message_format=text&color=red$/', $content);
    ...
    public function testWriteWithComplexMessage()
    {
        ...
        $this->assertRegexp('/messages=Backup\+of\+database\+%22example%22\+finished\+in\+16\+minutes\./', $content);
    }
    ...
    public function testWriteTruncatesLongMessage()
    {
        ...
        $this->assertRegexp('/messages='.str_repeat('abcde', 1900).'\+%5Btruncated%5D/', $content);
    }
    ...
    public function provideBatchRecords()
    {
        return array(
            array(
                array(
                    array('level' => Logger::WARNING, 'messages' => 'Oh bugger!', 'level_name' => 'warning', 'datetime' => new \DateTime()),
                    array('level' => Logger::NOTICE, 'messages' => 'Something noticeable happened.', 'level_name' => 'notice', 'datetime' => new \DateTime()),
                    array('level' => Logger::CRITICAL, 'messages' => 'Everything is broken!', 'level_name' => 'critical', 'datetime' => new \DateTime()),
                ),
                'red',
            ),
            array(
            ...
            array(
                array(
                    array('level' => Logger::DEBUG, 'messages' => 'Just debugging.', 'level_name' => 'debug', 'datetime' => new \DateTime()),
                    array('level' => Logger::NOTICE, 'messages' => 'Something noticeable happened.', 'level_name' => 'notice', 'datetime' => new \DateTime()),
                ),
                'green',
            ),
