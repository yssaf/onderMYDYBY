...
class RotatingFileHandlerTest extends TestCase
{
    ...
    private function assertErrorWasTriggered($code, $message)
    {
        if (empty($this->lastError)) {
            $this->fail(
                sprintf(
                    'Failed asserting that error with code `%d` and messages `%s` was triggered',
                    $code,
                    $message
                )
            );
        ...
        $this->assertEquals($message, $this->lastError['messages'], sprintf('Expected an error with messages `%d` to be triggered, got `%s` instead', $message, $this->lastError['messages']));
    }
