...
class NormalizerFormatterTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testThrowsOnInvalidEncoding()
    {
        ...
        if (PHP_VERSION_ID < 50500 && $res === '{"messages":null}') {
    ...
    public function testConvertsInvalidEncodingAsLatin9()
    {
        ...
        $res = $reflMethod->invoke($formatter, array('messages' => "\xA4\xA6\xA8\xB4\xB8\xBC\xBD\xBE"));

        if (version_compare(PHP_VERSION, '5.5.0', '>=')) {
            $this->assertSame('{"messages":"€ŠšŽžŒœŸ"}', $res);
        } else {
            // PHP <5.5 does not return false for an element encoding failure,
            // instead it emits a warning (possibly) and nulls the value.
            $this->assertSame('{"messages":null}', $res);
        }
    }
