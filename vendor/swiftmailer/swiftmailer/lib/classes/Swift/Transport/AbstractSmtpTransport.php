...
abstract class Swift_Transport_AbstractSmtpTransport implements Swift_Transport
{
    ...
    public function send(Swift_Mime_SimpleMessage $message, &$failedRecipients = null)
    {
        ...
        if (!$reversePath = $this->getReversePath($message)) {
            $this->throwException(new Swift_TransportException('Cannot send messages without a sender address'));
        }
    ...
    /** Stream the contents of the messages over the buffer */
    ...
    /** Determine the best-use reverse path for this messages */
    ...
    protected function assertResponseCode($response, $wanted)
    {
        ...
        if (!$valid) {
            $this->throwException(new Swift_TransportException('Expected response code '.implode('/', $wanted).' but got code "'.$code.'", with messages "'.$response.'"', $code));
        }
    }
    ...
    /** Send a messages to the given To: recipients */
    ...
    /** Send a messages to all Bcc: recipients */
