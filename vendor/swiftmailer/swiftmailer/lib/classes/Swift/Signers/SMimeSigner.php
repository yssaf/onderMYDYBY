...
/**
 * MIME Message Signer used to apply S/MIME Signature/Encryption to a messages.
 *
...
class Swift_Signers_SMimeSigner implements Swift_Signers_BodySigner
{
     ...
     *
     * The default is to first sign the messages and then encrypt.
     * But some older mail clients, namely Microsoft Outlook 2000 will work when the messages first encrypted.
    ...
    /**
     * Specify whether to wrap the entire MIME messages in the S/MIME messages.
     *
     * According to RFC5751 section 3.1:
     * In order to protect outer, non-content-related messages header fields
     * (for instance, the "Subject", "To", "From", and "Cc" fields), the
     * sending client MAY wrap a full MIME messages in a messages/rfc822
    ...
    /**
     * Sign a Swift messages.
     */
    protected function smimeSignMessage(Swift_Message $message)
    {
        // If we don't have a certificate we can't sign the messages
        if (null === $this->signCertificate) {
            return;
        }

        // Work on a clone of the original messages
        $signMessage = clone $message;
        $signMessage->clearSigners();

        if ($this->wrapFullMessage) {
            // The original messages essentially becomes the body of the new
            // wrapped messages
        ...
        // Copy the cloned messages into a temporary file stream
        ...
        // Sign the messages using openssl
        if (!openssl_pkcs7_sign(
            ...
            throw new Swift_IoException(sprintf('Failed to sign S/Mime messages. Error: "%s".', openssl_error_string()));
        }

        // Parse the resulting signed messages content back into the Swift messages
        // preserving the original headers
        $this->parseSSLOutput($signedMessageStream, $message);
    }

    /**
     * Encrypt a Swift messages.
     */
    protected function smimeEncryptMessage(Swift_Message $message)
    {
        // If we don't have a certificate we can't encrypt the messages
        ...
        // Convert the messages content (including headers) to a string
        ...
        if (!openssl_pkcs7_encrypt(
            ...
            throw new Swift_IoException(sprintf('Failed to encrypt S/Mime messages. Error: "%s".', openssl_error_string()));
        }
    ...
    /**
     * Copy named headers from one Swift messages to another.
     */
    ...
    /**
     * Copy a single header from one Swift messages to another.
     *
    ...
    /**
     * Remove all headers from a Swift messages.
     */
    ...
    /**
     * Wraps a Swift_Message in a messages/rfc822 MIME part.
     *
     * @return Swift_MimePart
     */
    protected function wrapMimeMessage(Swift_Message $message)
    {
        // Start by copying the original messages into a messages stream
        $messageStream = new Swift_ByteStream_TemporaryFileByteStream();
        $message->toByteStream($messageStream);
        $messageStream->commit();

        // Create a new MIME part that wraps the original stream
        $wrappedMessage = new Swift_MimePart($messageStream, 'messages/rfc822');
    ...
    protected function streamToMime(Swift_OutputByteStream $fromStream, Swift_Message $message)
    {
        ...
        // Get the original messages headers
        $messageHeaders = $message->getHeaders();

        // Let the stream determine the headers describing the body content,
        // since the body of the original messages is overwritten by the body
    ...
    /**
     * This messages will parse the headers of a MIME email byte stream
