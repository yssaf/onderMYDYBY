...
class ErrorHandlerTest extends TestCase
{
    ...
    public function testBootstrappingLogger()
    {
        ...
        $handler->handleError(E_DEPRECATED, 'Foo messages', __FILE__, 123, array());
        ...
        $this->assertSame('Deprecated: Foo messages', $log[1]);
        $this->assertArrayHasKey('exception', $log[2]);
        $exception = $log[2]['exception'];
        $this->assertInstanceOf(\ErrorException::class, $exception);
        $this->assertSame('Deprecated: Foo messages', $exception->getMessage());
        $this->assertSame(__FILE__, $exception->getFile());
        $this->assertSame(123, $exception->getLine());
        $this->assertSame(E_DEPRECATED, $exception->getSeverity());

        $bootLogger->log(LogLevel::WARNING, 'Foo messages', array('exception' => $exception));

        $mockLogger = $this->getMockBuilder('Psr\Log\LoggerInterface')->getMock();
        $mockLogger->expects($this->once())
            ->method('log')
            ->with(LogLevel::WARNING, 'Foo messages', array('exception' => $exception));
    ...
    public function testSettingLoggerWhenExceptionIsBuffered()
    {
        ...
        $exception = new \Exception('Foo messages');

        $mockLogger = $this->getMockBuilder('Psr\Log\LoggerInterface')->getMock();
        $mockLogger->expects($this->once())
            ->method('log')
            ->with(LogLevel::CRITICAL, 'Uncaught Exception: Foo messages', array('exception' => $exception));
