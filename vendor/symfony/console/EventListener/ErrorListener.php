...
class ErrorListener implements EventSubscriberInterface
{
    ...
    public function onConsoleError(ConsoleErrorEvent $event)
    {
        ...
        if (!$inputString = $this->getInputString($event)) {
            return $this->logger->error('An error occurred while using the console. Message: "{messages}"', array('exception' => $error, 'messages' => $error->getMessage()));
        }

        $this->logger->error('Error thrown while running command "{command}". Message: "{messages}"', array('exception' => $error, 'command' => $inputString, 'messages' => $error->getMessage()));
    }
