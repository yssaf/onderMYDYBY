...
class LoggerTest extends TestCase
{
    ...
    public function testFormatter()
    {
        $this->logger = new Logger(LogLevel::DEBUG, $this->tmpFile, function ($level, $message, $context) {
            return json_encode(array('level' => $level, 'messages' => $message, 'context' => $context)).\PHP_EOL;
        });

        $this->logger->error('An error', array('foo' => 'bar'));
        $this->logger->warning('A warning', array('baz' => 'bar'));
        $this->assertSame(array(
            '{"level":"error","messages":"An error","context":{"foo":"bar"}}',
            '{"level":"warning","messages":"A warning","context":{"baz":"bar"}}',
        ), $this->getLogs());
    }
}
